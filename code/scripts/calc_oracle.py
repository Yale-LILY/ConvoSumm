...
def cal_rouge(fullset, sentdata, golddata):
    ...
    rouge_1 = rouge_n(model_highlights, golddata, 1)['f']
    rouge_2 = rouge_n(model_highlights, golddata, 2)['f']
    rouge_l = rouge_l_summary_level(model_highlights, golddata)['f']
...
if __name__ == "__main__":
    ...
    domain = sys.argv[1]
    source = f"./{domain}.test.source.remove_markers_simple_separator"
    target = f"./{domain}.test.target.remove_markers_simple_separator"
    pool = Pool(24)
    ...
    with open(source) as srcf, open(target) as tgtf, open(source + ".oracle", "w") as outputf:
        for count, (srcl, tgtl) in enumerate(zip(srcf, tgtf)):
            print(count)
            sentdata = [x.text for x in nlp(srcl.strip()).sents]
            golddata = [x.text for x in nlp(tgtl.strip()).sents]
            ...
            arguments_list = []
            for itemcount in range(1, sent_limit + 1):
            ...
            try:
              out_sentids = rougescore_sentids[0][1]
            except:
              out_sentids = [0]
            try:
                out_sentences = [sentdata[x] for x in out_sentids]
            except:
                import pdb;pdb.set_trace()
            out_sentences = " ".join(out_sentences)
            outputf.write(out_sentences + "\n")
