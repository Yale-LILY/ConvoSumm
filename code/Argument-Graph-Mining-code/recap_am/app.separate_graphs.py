...
def _process(filename, statistic: Statistic, out_path: Path) -> None:
    ...
    with open(filename) as inputf, open(filename + ".graph", "w") as outputf:
        for count, line in enumerate(inputf):
            start_time = timer()
            print(count)
            info = line.split("</s>")[0] # TODO: assumes that there is information such as forum name, question tags, which should be included in the source text but not as part of the graph
            queries = []
            subject_info = []
            for email in line.split("</s>")[1:]:
            ...
            for query_count, query_doc in enumerate(queries):
                ...
                subject_node = ag.Node(key=str(query_count) + "-" + str(cur_graph.keygen()), text=subject_info[query_count] + " <s> ", category=ag.NodeCategory.I)
                subject_nodes.append(subject_node)
                for claim_sent in cur_claim_sents:
                    claim_node = cur_claim2node[str(claim_sent)]
                    edge_1 = ag.Edge(key=str(query_count) + "-" + str(cur_graph.keygen()), start=subject_node, end=claim_node)
                    all_edges.append(edge_1)

            graph_end2end = ag.Graph(name=doc._.key.split("/")[-1])
            conversation_node = ag.Node(key="final-" + str(graph_end2end.keygen()), text="Conversation", category=ag.NodeCategory.I)
            for subject_count, subject_node in enumerate(subject_nodes):
                edge_1 = ag.Edge(key="final-" + str(query_count) + "-" + str(graph_end2end.keygen()), start=conversation_node, end=subject_node)
                all_edges.append(edge_1)
            ...
            for edge in gedges:
                ...
                #    output_text += "Claim: " + n2_text + " "
                #elif n1_text == "Issue": 
                #    output_text += "Issue: " + n2_text + " "
                #elif n1_text == "Default Inference":
                #    output_text += "is entailed by: " + n2_text + " "
                #elif n2_text == "Default Inference":
                #    output_text += "Claim: " + n1_text + " "
                #else:
                #    output_text += "Claim: " + n1_text + " is entailed by " + n2_text + " "
                elif n1_text == "Conversation":
            ...
            output_text = info + " </s> " + output_text
